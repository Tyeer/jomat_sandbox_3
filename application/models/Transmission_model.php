...
class Transmission_model extends CI_Model {
	...
	public function getTransmission($filters=array())
	{
	        //reset variables
			$this->reset_defaults();
			...
			{ 	
				$filteringTransmission=$filters['filtering_condition'];
				...
				else
				{
			   	...
			   	foreach($valueFormated as $key2 => $value2)
			   	{
					if($key==='transmission_id' || ($key==='transmission_id'.$n) )
					{
						...
						$n++;
					}elseif($key==='transmission_name' || ($key==='transmission_name'.$n) )
					{
			...
			$WheresubQuery="";
			$filteringTransmission=' '.$filteringTransmission.' ';
			if(!empty($WhereSubQueryArry))
			$WheresubQuery='WHERE '.implode($filteringTransmission,$WhereSubQueryArry);			 
					...
					if(is_array($value))
					 ...
					 else
				 	{
				   	...
				   	{					   
						...
						if($key==='transmission_name' || ($key==='transmission_name'.$n) ) 
				...
				if($dbquery1->execute())
			   	{
					...
					if(count($sql_result)>0)
					{
					    foreach( $sql_result as $key => $value)
						{
							...
							//do some format
							$result[$key]=array(									
												"transmission_id"=>$value['id'],
												"transmission_name"=>$value['name'],
												"transmission_rank"=>$value['rank'],
												);
						}
					}
